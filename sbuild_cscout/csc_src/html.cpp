...
// Return a function's label, based on the function type
string
type_function_label(Call *f, bool hyperlink)
{
    char previous_option = Option::cgraph_show->get();
    Option::cgraph_show->set_directly('p');
    string res = function_label(f, hyperlink);
    Option::cgraph_show->set_directly(previous_option);
    if (f->is_file_scoped())
        res.insert(0, "static:");
    else
        res.insert(0, "public:");
