...
// Raw text output with more information about function types
// Static functions have full path in nodes (cgraph_show: p)
// Public functions have only function name in nodes (cgraph_show: n)
class GDTxtFtype: public GraphDisplay {
public:
	GDTxtFtype(FILE *f) : GraphDisplay(f) {}
	virtual void edge(Call *a, Call *b) {
		fprintf(fo, "%s %s\n",
		    type_function_label(a, false).c_str(),
		    type_function_label(b, false).c_str());
	}
